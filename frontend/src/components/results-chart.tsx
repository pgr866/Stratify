...
export function ResultsChart({ absCumProfit, relCumProfit, absDrawdown, relDrawdown, absHodlingProfit, relHodlingProfit }) {
  ...
  const getChartOptions = () => ({
    ...
    rightPriceScale: {
      ...
      borderVisible: false,
      alignLabels: false,
      scaleMargins: { bottom: 0.03, top: 0.03 },
    },
    leftPriceScale: {
      visible: true,
      borderVisible: false,
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (!chartInstance.current) return;
      if (absCumProfit?.length || absDrawdown?.length || relDrawdown?.length || absHodlingProfit?.length || relHodlingProfit?.length) return;
      chartInstance.current.applyOptions({
        ...getChartOptions(),
        rightPriceScale: {
          ...getChartOptions().rightPriceScale,
          visible: false,
        ...
        localization: {
          ...
          ...(selectedTab === 'percentage' && {
            priceFormatter: (value) =>
              new Intl.NumberFormat(window.navigator.languages[0], {
                style: 'percent',
      ...
      series.current.cumProfit.setData(relCumProfit.map((v, i) => ({ time: i + 1, value: v })));
      chartInstance.current.timeScale().fitContent();
    }, 0);
    return () => clearTimeout(timeout);
  }, [selectedTab, relCumProfit]);
  ...
  return (
    <div className="flex flex-col flex-1 h-full overflow-hidden">
      ...
      <div className="w-full h-fit flex flex-none items-center gap-4 justify-between px-4">
        {(absCumProfit?.length || absDrawdown?.length || relDrawdown?.length || absHodlingProfit?.length || relHodlingProfit?.length) && (
          <div className="w-full h-fit flex flex-none items-center gap-4 justify-between px-4">
            ...
            <Tabs defaultValue={selectedTab} onValueChange={setSelectedTab} className="h-auto ml-auto">
